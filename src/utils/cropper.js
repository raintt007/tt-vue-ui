export default {
  ...
  validate: function(filesArr, configs) {
    ...
    filesArr.map((file, index) => {
      ...
      if (
        configs.filetype &&
        !new RegExp(`(${configs.filetype.value})$`, "i").test(file.type)
      ) {
        alertText = alertText.concat(
          ` 第${index + 1}张` + configs.filetype.message
        );
      }
      ...
      if (configs.filesize && file.size > configs.filesize.value) {
        alertText = alertText.concat(
          ` 第${index + 1}张` + configs.filesize.message
        );
      }
    });
    if (alertText != "") {
  ...
  adviceValidate: function(filesArr, configs) {
    ...
    filesArr.map((file, index) => {
      // 格式校验
      if (
        configs.adviceFiletype &&
        !new RegExp(`(${configs.adviceFiletype.value})$`, "i").test(file.type)
      ) {
  ...
  moveTrimPosition: function(
    ...
    switch (option) {
      ...
      case 4: // w
        tempStartX =
          currentX - (startX + width) > 0 ? startX + width : currentX;
